...
8.在路由守卫处，使用pinia的useUserStore()来获取用户信息。这样就是一个响应式的数据，可以在路由守卫中使用。注：改方法和localStorage的安全性相同，都是将数据存储在浏览器端，不会被服务器端访问。区别是，pinia的响应式数据可以在组件中直接使用，而localStorage需要通过JSON.parse()来解析。
9.当网络断网的时候，也是会出现401报错，这个时候应该添加可选链操作符，来避免报错。
...
## 学习目标清单（按难度递增）
### 🌟 目标 1：表单校验体系化（Element Plus 表单规则）
现状 ：你的 UserManage.vue 第 77-92 行的 handleSubmit 是 手动 if 判断 校验：

学习点 ：

- 用 Element Plus 的 el-form + :rules + ref 调用 formRef.validate()
- 异步校验器（比如校验"用户名是否已存在"要调接口）
- 自定义校验函数（密码强度、邮箱格式）
为什么值得学 ：这是 Element Plus 中高级用法，企业项目里表单校验几乎都是这套模式。手动 if 判断在字段多时会非常冗长。

### 🌟🌟 目标 2：环境变量与多环境配置（Vite）
现状 ：你的 request.ts 第 8 行硬编码了 baseURL: 'http://localhost:3001' 。

学习点 ：

- 在项目根目录建 .env.development 和 .env.production
- 用 import.meta.env.VITE_API_BASE_URL 读取
- 理解 Vite 的环境变量约定（必须 VITE_ 前缀）
- vite preview 模拟生产环境构建
为什么值得学 ：硬编码 baseURL 在部署时是地雷——开发用 localhost，生产用真实域名，每次都改代码会出错。这是工程化的第一步。

### 🌟🌟 目标 3：路由懒加载 + 代码分割
现状 ：你的 router/index.js 是 静态 import 所有页面：

学习点 ：

- 改成 () => import('@/views/Login.vue') 动态导入
- 理解 Webpack/Vite 的代码分割（chunk）
- 用 vite build 后看 dist 目录，对比打包体积差异
- 给每个 chunk 命名（ /* webpackChunkName: "login" */ ）
为什么值得学 ：项目大了之后首屏加载速度会成问题。懒加载能让首屏只加载必要代码，其他页面按需加载。这是性能优化的基本功。

### 🌟🌟🌟 目标 4：Pinia 持久化插件 + store 模块化
现状 ：你的 useLoginUserStore.js 是手动从 localStorage 读写：

学习点 ：

- 安装 pinia-plugin-persistedstate 插件
- 在 store 定义里加 persist: true 即可自动持久化
- 拆分多个 store： useUserStore 、 useAppStore （主题、侧边栏折叠状态）、 usePermissionStore （动态路由权限）
- store 之间互相调用（ useAppStore 里调 useUserStore ）
为什么值得学 ：手动持久化在字段多了后维护成本极高（每改一个字段都要同步 localStorage）。插件化是工业级方案。

### 🌟🌟🌟🌟 目标 5：动态路由 + 权限菜单（RBAC）
现状 ：你的路由表是 静态写死的 ，所有路由都写在前端，只是用 meta.requestAdmin 做守卫拦截。

学习点 ：

- 后端返回当前用户的"可访问路由列表"
- 前端用 router.addRoute() 动态注册路由
- 侧边栏菜单根据"动态路由"渲染（admin 看到全部，user 只看到部分）
- 处理刷新页面后动态路由丢失的问题（要在守卫里等路由添加完再放行）
为什么值得学 ：这是企业级中后台系统的核心架构。你现在的"前端写死路由 + meta 标记"是简化版，真实项目里权限是后端控制的。学完这个你就具备做中后台的能力了。

关键坑 ：动态路由最大的坑就是"刷新丢失"——刷新页面时 Vue 应用重启，动态路由没了，守卫又依赖路由表判断权限，会陷入死循环。解决思路是在守卫里判断"如果没有动态路由，先等 addRoute 完成再 next({ ...to, replace: true }) "。

### 🌟🌟🌟🌟🌟 目标 6：TypeScript 全面迁移
现状 ：你的项目是 JS 为主 ，只有 request.ts 是 TS（但里面也没用类型）。

学习点 ：

- 给所有 API 函数加类型： userLogin(data: LoginParams): Promise<ApiResponse<LoginResult>>
- 给 store 加类型： defineStore('user', (): UserInfo => { ... })
- 给组件 props 加类型： defineProps<{ id: number; mode: 'edit' | 'add' }>()
- 用 vue-tsc 做类型检查
- 泛型接口设计（ ApiResponse<T> 这种通用响应类型）
为什么值得学 ：TS 是前端进阶的必经之路。你已经在用 TS 文件了（ request.ts ），但没用类型等于没用 TS。真正用起来后，改一个接口签名，所有调用处都会报红，重构成本大幅降低。